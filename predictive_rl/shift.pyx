...
@cython.boundscheck(False)
@cython.wraparound(False)
def shift2d_uint8(ar[np.uint8_t,ndim=2] data, int shift_amt):
    cdef int i, j
    cdef int n = data.shape[0]
    cdef int h = data.shape[1]
    ...
    for i in xrange(shift_amt, n):
        for j in xrange(h):
                data[i-shift_amt, j] = data[i, j]

@cython.boundscheck(False)
@cython.wraparound(False)
def shift2d_int32(ar[np.int32_t,ndim=2] data, int shift_amt):
    cdef int i, j
    cdef int n = data.shape[0]
    cdef int h = data.shape[1]
...
@cython.boundscheck(False)
@cython.wraparound(False)
def shift2d_float32(ar[np.float32_t,ndim=2] data, int shift_amt):
    cdef int i, j
    cdef int n = data.shape[0]
    cdef int h = data.shape[1]
