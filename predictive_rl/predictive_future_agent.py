...
class PredictiveFutureAgent(ExperimenterAgent):
    ...
    def exp_step(self, reward, observation, is_testing):
        ...
        if not is_testing:
            loss = self._do_training(np.asmatrix(reward, dtype=floatX), cur_observation, double_action, False,
                                     pred_observation_value, pred_observation, pred_action)
    ...
    def _predict(self, observation):
        ...
        next_observation_value = pred_matrix[:, self.action_size + self.observation_size]
        return next_action, next_observation, next_observation_value
